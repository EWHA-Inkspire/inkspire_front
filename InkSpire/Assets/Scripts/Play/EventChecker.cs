...
public class EventChecker : MonoBehaviour {
    ...
    public async Task<bool> EventCheckerGPT(string assistant_msg, string input_msg, Event game_event) {
        ...
        {
            Role = "system",
            Content = @"주어진 이벤트 트리거, 이벤트 제목, 도입 스크립트, 시나리오, 플레이어 선택 텍스트를 분석하여 이벤트 발생 여부를 출력하시오. 이벤트 발생 여부는 플레이어 선택이 이벤트 트리거 근처에 도달하는 구체적인 행동이 있어야 발생한다. 이 때 답변은 반드시 T 혹은 F로 해야한다. 이 때 답변은 반드시 T 혹은 F로 해야한다.
            아래는 출력 예시이다.
