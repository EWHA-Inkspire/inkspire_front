...
public class Event
{
    ...
    public async Task CreateEventTrigger(string world_detail, string chapter_obj, string place_name, string item_name)
    {
        ...
        // (event_type == 1 ? "챕터 목표에 어울리는 발견 위치를 보통명사로 생성한다.\n챕터 목표: " + chapter_obj + "\n" : "") + 

        var prompt_msg = new ChatMessage()
        {
            Role = "system",
            Content = @"당신은 게임 아이템의 발견 위치를 생성한다. " + "게임의 세계관 배경은 다음과 같다. " + world_detail
...
ex 1)
이벤트제목: 은혜당의 구미호를 만나다.
도입: 당신은 은혜당 안에서 아름다운 여인이 떠돌아다니는 모습을 발견했습니다. 그녀는 아찔한 아름다움을 띤 채로 당신을 쳐다보며 당신에게 손짓을 합니다. 당신은 그녀가 구미호일지도 모른다고 생각하며 조심스럽게 다가갑니다.
성공: 여인은 당신을 환한 미소로 보면서 손에 삼다을을 쥐여줍니다. 획득한 삼다을은 당신의 회복 능력을 상승시켜줄 것입니다. 
실패: 당신이 그 여인에게 다가가려고 하자, 여인은 흔적도 없이 홀연히 모습을 감춥니다. 정말 구미호였던 걸까요? 당신은 의문을 가진 채 탐험을 이어갑니다.  

ex 2)
이벤트제목: 신비로운 초승달
도입: 당신은 퍼플릿 마너에 도착하여 하늘을 바라봅니다. 하늘에는 초승달이 떠 있습니다. 그 순간 이상한 느낌이 들더니 땅 속에서 빛나는 물체를 발견합니다. 그것은 고대 문양이 새겨진 작은 광석 조각입니다. 당신이 이 물체에 손을 대자, 갑자기 시공을 넘나드는 느낌이 밀려옵니다.
성공: 당신의 머리 속에 이상한 기호와 미로 모양의 이미지가 번뜩입니다. 이것은 고대 마법의 암호로, 당신은 이를 해독할 수 있는 새로운 혈형식을 얻습니다.
실패: 하지만 그 느낌도 잠시, 당신은 아무 일도 없었던 것처럼 다시 원래의 시공간으로 돌아옵니다. 알 수 없는 이상한 경험을 뒤로하고, 당신은 탐험을 이어갑니다.  

ex 3)
이벤트제목: 심곡류의 비밀
도입: 당신이 은밀한 책방을 둘러보던 중, 한 권의 특별한 책이 눈에 띕니다. 책을 확인해보니 그 안에는 심곡류에 대한 이야기가 적혀있습니다. 심곡류는 홍익가에서 유명한 식재료로, 그 신비한 효능이 전해지고 있습니다. 하지만 책 속의 내용은 쉽게 이해하기 어렵고, 당신은 이것이 홍익가의 미스터리에 연결되어 있을지도 모른다는 생각이 듭니다.
성공: 당신은 책의 한 구석에서 누군가 심곡류에 대한 내용을 해독해놓은 글귀를 발견합니다. 글귀를 읽은 당신은 심곡류의 효능에 대한 설명과 함께, 이 식재료가 어떻게 가공되고 사용되는지, 그리고 그것이 홍익가의 다양한 요리에 어떤 영향을 미치는지에 대한 내용을 알게 됩니다. 어쩌면 이 내용이 홍익가의 비밀을 해결하는 데 결정적인 역할을 할지도 모르겠습니다!
실패: 당신은 책방을 뒤지던 중 실수로 책 한 권을 바닥에 떨어뜨립니다. 책의 내용이 물에 젖어 손상되어 안타깝게도 당신은 심곡류의 비밀을 알아낼 수 없게 되었습니다. 아쉬운 마음으로 당신은 탐험을 이어갑니다.";
        gpt_messages.Add(prompt_msg);

        query_msg.Content = @"아래와 같은 양식으로 판정 이벤트를 생성한다. ** 이 표시 안의 내용은 문맥에 맞게 채운다.
        
이벤트제목: *이벤트의 제목을 출력*
도입: *이벤트 트리거를 건드렸을 때 판정이벤트가 발생하며 출력될 게임 시나리오 스크립트 출력* 
성공: *이벤트 판정에 성공했을 때 출력될 아이템 획득 게임 시나리오 스크립트*
실패: *이벤트 판정에 실패했을 때 출력될 게임 시나리오 스크립트*";
        gpt_messages.Add(query_msg);
